...
class Framerate extends Sprite {
	...
	public function new() {
		...
		instance = this;
		textFormat = new TextFormat(Assets.getFont("assets/fonts/consola.ttf").fontName, 12, -1);
		fontName = Assets.getFont("assets/fonts/consola.ttf").fontName;
	...
	public override function __enterFrame(t:Int) {
		...
		var width = Math.max(fpsCounter.width, Math.max(memoryCounter.width, codenameBuildField.width)) + (x*2);
		var height = codenameBuildField.y + codenameBuildField.height;
		bgSprite.x = -x;
		bgSprite.y = offset.x;
		...
		var selectable = debugMode == 2;
		{
			...
			codenameBuildField.codenameTextField.selectable = selectable;
		}
