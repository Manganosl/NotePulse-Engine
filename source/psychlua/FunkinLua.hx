...
class FunkinLua {
	...
	public function new(scriptName:String, isString:Bool = false) {
		...
		Lua_helper.add_callback(lua, "fpsLerp", function(from:Float, to:Float, lerpSpeed:Float) { // returns the global from a script
			return CoolUtil.fpsLerp(from, to, lerpSpeed);
		});
		...
		Lua_helper.add_callback(lua, "initNdll", function(tag:String, ndllPath:String, name:String, args:Int){
			var ndll = NdllUtil.getFunction(ndllPath, name, args);
			game.modchartNdlls.set(tag, ndll);
		});
		...
		Lua_helper.add_callback(lua, "addLuaBackdrop", function(tag:String, front:Bool = false) {
			if(game.modchartBackdrops.exists(tag)) {
				var shit:ModchartBackdrop = game.modchartBackdrops.get(tag);
				if(!shit.wasAdded) {
					if(front)
					{
		...
		Lua_helper.add_callback(lua, "windowTweenX", function(tag:String, value:Float, duration:Float, ease:String = 'linear') {
			#if windows
			PlayState.instance.modchartTweens.set(tag, FlxTween.tween(Application.current.window, {x: value}, duration, {ease: LuaUtils.getTweenEaseByString(ease), onComplete:
		...
		Lua_helper.add_callback(lua, "windowTweenY", function(tag:String, value:Float, duration:Float, ease:String = 'linear') {
			#if windows
			PlayState.instance.modchartTweens.set(tag, FlxTween.tween(Application.current.window, {y: value}, duration, {ease: LuaUtils.getTweenEaseByString(ease), onComplete:
				function(twn:FlxTween){
					PlayState.instance.callOnLuas('onTweenCompleted', [tag]);
					PlayState.instance.modchartTweens.remove(tag);
				}
