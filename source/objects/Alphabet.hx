...
class Alphabet extends FlxSpriteGroup
{
	...
	public static var centeredChangeX:Bool = true;
	...
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = true, ?isCentered:Bool = false)
	{
		super(x, y);
	...
	public static var lerpSelected:Float = 0;
override function update(elapsed:Float)
{
    if (isMenuItem)
    {
        ...
        // Target Y position, like your updateTexts()
        var targetPosY:Float = ((FlxG.height - 120) / 2) + ((targetY - lerpSelected) * 135);
        
        // Smooth Y movement
        y = FlxMath.lerp(targetPosY, y, lerpVal);

        if (isCentered && centeredChangeX)
        {
            var targetPosX:Float = (FlxG.width - this.width) / 2;
            x = FlxMath.lerp(targetPosX, x, lerpVal);
        }
        else
        {
...
public function snapToPosition()
{
    if (isMenuItem)
    {
        y = ((FlxG.height - 120) / 2) + ((targetY - lerpSelected) * 135);
