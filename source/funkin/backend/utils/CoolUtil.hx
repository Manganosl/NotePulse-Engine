...
class CoolUtil
{
	...
	public static inline function smoothLerpPrecision(current:Float, target:Float, elapsed:Float, duration:Float, precision:Float = 0.99):Float {
		if (duration <= 0) return target;
		return FlxMath.lerp(current, target, 1 - Math.pow(1 - precision, elapsed / duration));
	}

	public static inline function snap(value:Float, target:Float, threshold:Float):Float
