package funkin.game.modchart.backend.macros;
...
class ModifiersMacro {
	public static macro function get():ExprOf<Iterable<Class<Dynamic>>> {
		...
		var request:String = 'package~${'funkin.game.modchart.engine.modifiers.list'}~recursive';

		classListsToGenerate.push(request);

		return macro funkin.game.modchart.backend.macros.CompiledClassList.get($v{request});
	}
	...
	static function compileClassLists() {
		var compiledClassList:ClassType = getClassType("funkin.game.modchart.backend.macros.CompiledClassList");
