...
class AlphaModifier extends NoteModifier
{
	...
	public function getHiddenSudden(player:Int = -1, column:Int = -1)
		return MathUtil.clamp(getWithColumnVariant("hidden", player, column) + getWithColumnVariant("sudden", player, column), 0, 1);
	...
	function getVisibility(yPos:Float, player:Int, column:Int):Float {
		...
		if (yPos < 0 && getSubmodValue("stealthPastReceptors", player) == 0){
			var flatStealth = getValue(player) + getSubmodValue('stealth$column', player);
			return MathUtil.clamp(1 - flatStealth, 0, 1);
		}
