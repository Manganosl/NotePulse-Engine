...
import funkin.game.modchart.backend.core.Node.NodeFunction;
import funkin.game.modchart.backend.core.VisualParameters;
import funkin.game.modchart.backend.graphics.CtxRenderer;
import funkin.game.modchart.backend.graphics.DrawCommand;
 ...
 * - Rendering
 */
@:allow(funkin.game.modchart.backend.ModifierGroup)
@:access(funkin.game.modchart.engine.ModPlayField)
#if !openfl_debug
...
final class Manager extends FlxBasic {
	...
	public inline function easeFunc(beat:Float, length:Float, ease:EaseFunction, callback:(Event, Float, Float) -> Void)
		iteratePlayfields((pf) -> pf.easeFunc(beat, length, callback, ease), 0);

	public inline function queueEaseFunc(step:Float, stepToEnd:Float, ease:EaseFunction, callback:(Event, Float, Float) -> Void)
		easeFunc(stepToBeat(step), stepToBeat(stepToEnd - step), ease, (ev, val, beat) -> callback(ev, val, beat * 4));
