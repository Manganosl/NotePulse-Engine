...
import funkin.psychlua.GlobalHandler;
...
class ModSelector extends MusicBeatState {
    ...
    override public function create() {
        ...
        changeSelection();
        updateTexts();
        if (goto == funkin.states.editors.ChartingState || goto == funkin.states.editors.ModchartEditor) reloadSongs();
    ...
    override public function update(elapsed:Float) {
        ...
        if (grpAlph.length >= 1) {
            ...
            if (controls.ACCEPT) {
                ...
                if (goto != funkin.states.editors.ChartingState && goto != funkin.states.editors.ModchartEditor) {
                    if (curSelected == 0) {
                        currentMod = null;
                    } else {
                        currentMod = modArray[curSelected];
                ...
                } else if (goto == funkin.states.editors.ChartingState || goto == funkin.states.editors.ModchartEditor) {
                    if (inDifSelect) {
                        if (curSelected == 0) {
...
class GoodBye extends MusicBeatSubstate {
	override public function create(){
		...
		add(warningText);
		new FlxTimer().start(5, function(tmr:FlxTimer){ try MusicBeatState.switchState(new funkin.states.editors.WeekEditorState()); close();});
	}
}