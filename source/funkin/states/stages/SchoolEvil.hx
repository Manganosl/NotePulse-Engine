...
class SchoolEvil extends BaseStage
{
	override function create()
	{
		...
		if(!ClientPrefs.data.lowQuality)
			bg = new BGSprite('base_game/weeb/animatedEvilSchool', posX, posY, 0.8, 0.9, ['background 2'], true);
		else
			bg = new BGSprite('base_game/weeb/animatedEvilSchool_low', posX, posY, 0.8, 0.9);
		...
		FlxG.sound.playMusic(Paths.music('base_game/LunchboxScary'), 0);
	...
	override function eventPushed(event:funkin.objects.notes.Note.EventNote)
	{
		...
		switch(event.event)
		{
			case "Trigger BG Ghouls":
				if(!ClientPrefs.data.lowQuality)
				{
					bgGhouls = new BGSprite('base_game/weeb/bgGhouls', -100, 190, 0.9, 0.9, ['BG freaks glitch instance'], false);
			...
			if (doof != null)
			{
				...
				new FlxTimer().start(0.3, function(swagTimer:FlxTimer)
				{
					...
					else
					{
						...
						FlxG.sound.play(Paths.sound('base_game/Senpai_Dies'), 1, false, null, true, function()
						{
							remove(senpaiEvil);
							senpaiEvil.destroy();
							remove(red);
