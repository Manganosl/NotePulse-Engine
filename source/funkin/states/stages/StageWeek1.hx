...
class StageWeek1 extends BaseStage
{
	...
	override function create()
	{
		...
		if(!ClientPrefs.data.lowQuality) {
			var stageLight:BGSprite = new BGSprite('base_game/stage_light', -125, -100, 0.9, 0.9);
			stageLight.setGraphicSize(Std.int(stageLight.width * 1.1));
			stageLight.updateHitbox();
			add(stageLight);
			var stageLight:BGSprite = new BGSprite('base_game/stage_light', 1225, -100, 0.9, 0.9);
			stageLight.setGraphicSize(Std.int(stageLight.width * 1.1));
			stageLight.updateHitbox();
			stageLight.flipX = true;
			add(stageLight);

			var stageCurtains:BGSprite = new BGSprite('base_game/stagecurtains', -500, -300, 1.3, 1.3);
	...
	override function eventPushed(event:funkin.objects.notes.Note.EventNote)
	{
		switch(event.event)
		{
			case "Dadbattle Spotlight":
				...
				dadbattleLight = new BGSprite('base_game/spotlight', 400, -400);
