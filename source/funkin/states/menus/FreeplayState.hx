...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			var skipSong:Bool = false;
			if (Mods.currentLoadedMod != null && Mods.currentLoadedMod.length > 0) 
			{
				if (songs[i].folder != Mods.currentLoadedMod) 
				{
		...
		rankText = new FlxText(FlxG.width * 0.7, 5, 0, "P", 32);
		rankText.setFormat(Paths.font("Phan.ttf"), 80, FlxColor.WHITE, CENTER);
		add(rankText);

		scoreBG = new FlxSprite(scoreText.x - 36, 0).makeGraphic(1, 66, 0xFF000000);
		scoreBG.alpha = 0;
		add(scoreBG);
	...
	override function update(elapsed:Float)
	{
		...
		if (!player.playingMusic)
		{
			...
			rankText.text = getRank(songs[curSelected].songName, curDifficulty, Std.parseFloat(ratingSplit.join('.')));
			positionHighscore();
		...
		else if(FlxG.keys.justPressed.SPACE && controlsActive)
		{
		...
		else if (controls.ACCEPT && !player.playingMusic && controlsActive)
		{
			...
			//FlxTween.tween(iconArray[curSelected], {x: iconArray[curSelected].x + 1500}, 0.8, {ease: FlxEase.backIn, type: PINGPONG});
		...
		else if(controls.RESET && !player.playingMusic && controlsActive)
		{
			persistentUpdate = false;
	...
	private function getRank(song:String, diff:Int, rating:Float):String {
		if(Highscore.getFC(song, diff) != false && intendedScore-10 <= lerpScore) return "P";
		else if(rating >= 100) return "P";
		else if(rating >= 90) return "S";
		else if(rating >= 70) return "A";
		else if(rating >= 60) return "B";
		else if(rating >= 50) return "C";
		else if(rating >= 40) return "D";
		else if(rating >= 20) return "F";
		else return "L";
	}
	...
	private function positionHighscore() {
		if (rankText.text != null){
			rankText.x = FlxG.width - rankText.width - 16;
			scoreText.x = FlxG.width - scoreText.width - 86;
		} else
			scoreText.x = FlxG.width - scoreText.width - 6;
		scoreBG.scale.x = FlxG.width - scoreText.x + 6;
		scoreBG.x = FlxG.width - (scoreBG.scale.x / 2);
		diffText.x = Std.int(titleText.x + (titleText.width / 17)+10);
