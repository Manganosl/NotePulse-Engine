...
class SpectogramSprite extends FlxTypedSpriteGroup<FlxSprite>
{
	...
	public function new(daSound:FlxSound, ?col:FlxColor = FlxColor.WHITE, ?height:Float = 720, ?amnt:Int = 500, ?dir:SPECDIRECTION = VERTICAL)
	{
		super();
	...
	public function regenLineShit():Void
	{
		...
		{
			var lineShit:FlxSprite = new FlxSprite().makeGraphic(1, 1, col);
			switch (direction)
			{
				case VERTICAL:
					lineShit.setPosition(100, i / lengthOfShit * daHeight);
				case HORIZONTAL:
					lineShit.setPosition(i / lengthOfShit * daHeight, 100);
			}
	...
	public function updateFFT()
	{
		if (vis.snd != null)
		{
			...
			if (setBuffer && doAnim)
			{
				...
				else remappedShit = Std.int(FlxMath.remapToRange(Conductor.songPosition, 0, vis.snd.length, 0, numSamples));
				...
				for (i in 0...group.members.length)
				{
					...
					switch (direction)
					{
						case VERTICAL:
							...
							prevLine.y = (i / group.members.length * daHeight) + y;
						case HORIZONTAL:
							prevLine.x = (i / group.members.length * daHeight) + y;
							prevLine.y = (freqIDK * swagheight / 2 + swagheight / 2) + x;
					}
			...
			if (setBuffer)
			{
				...
				for (sample in remappedShit...remappedShit + lengthOfShit)
				{
					...
					switch (direction)
					{
						...
						case HORIZONTAL:
							prevLine.x = (Std.int(remappedSample) / lengthOfShit * daHeight) + y;
							prevLine.y = (curAud.balanced * swagheight / 2 + swagheight / 2) + x;
					}
