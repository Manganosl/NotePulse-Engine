...
class PauseSubState extends MusicBeatSubstate
{
	...
	var menuItemsOG:Array<String> = ['Resume', 'Restart Song', 'Change Player', 'Change Difficulty', 'Options', 'Exit to menu'];
	...
	override function update(elapsed:Float)
	{
		...
		velXtra = CoolUtil.fpsLerp(velXtra, 0, 0.1);
		...
		{
			if(item == skipTimeTracker && item.targetY == 0)
			{
				var lerpVal:Float = Math.exp(-elapsed * 9.6);
            	item.x = FlxMath.lerp(10, item.x, lerpVal);
			}
		}
	}
	...
	function changeSelection(change:Int = 0):Void
	{
		...
		for (item in grpMenuShit.members)
		{
			...
			if (!Alphabet.centeredChangeX) Alphabet.centeredChangeX = true;
			...
			if (item.targetY == 0)
			{
				...
				{
					Alphabet.centeredChangeX = false;
	...
	function regenMenu():Void {
		...
		for (i in 0...menuItems.length) {
			var item = new Alphabet(90, 320, menuItems[i], true, true);
	...
	function updateSkipTextStuff()
	{
		...
		skipTimeText.x = Math.round(skipTimeTracker.x + skipTimeTracker.width + 60);
