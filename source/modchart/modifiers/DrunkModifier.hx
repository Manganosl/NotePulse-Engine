...
class DrunkModifier extends NoteModifier {
	...
	inline function applyDrunk(axis:String, player:Int, time:Float, visualDiff:Float, data:Float, ?mathFunc:Float->Float){
		...
		if(perc!=0){
			...
			return perc * (mathFunc(angle) * (Note.swagWidth * 0.5));
		}
		return 0;
	}
	...
	inline function applyTipsy(axis:String, player:Int, time:Float, visualDiff:Float, data:Float, ?mathFunc:Float->Float)
	{
		...
		var perc = getSubmodValue('tipsy${axis}', player) + getSubmodValue('tip${axis}', player);
		var speed = getSubmodValue('tipsy${axis}Speed', player) + getSubmodValue('tip${axis}Speed', player);
		var offset = getSubmodValue('tipsy${axis}Offset', player) + getSubmodValue('tip${axis}Offset', player);

		if (perc != 0)
			return perc * (mathFunc((time * ((speed * 1.2) + 1.2) + data * ((offset * 1.8) + 1.8))) * Note.swagWidth * .4);
...
override function getPos(time:Float, visualDiff:Float, timeDiff:Float, beat:Float, pos:Vector3, data:Int, player:Int, obj:FlxSprite)
	{
		...
		pos.z += 
			...
			+ applyBumpy("", player, time, visualDiff, data)) / 1280;
		...
		pos.z += 
			...
			+ applyBumpy('$data', player, time, visualDiff, data)) / 1280;
		...
		pos.z += 
			...
			+ applyBumpy("Tan", player, time, visualDiff, data, Math.tan)) / 1280;
		...
		pos.z += 
			...
			+ applyBumpy('Tan$data', player, time, visualDiff, data, Math.tan)) / 1280;
	...
	override function getSubmods(){
		...
		var shids:Array<String> = ["drunk","tipsy","tip","bumpy","drunkTan","tipsyTan","bumpyTan"];
