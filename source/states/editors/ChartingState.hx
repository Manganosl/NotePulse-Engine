...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function createGrids()
	{
		...
		gfLockOverlay = new FlxSprite(gridBg.x+(GRID_SIZE*(1+GRID_COLUMNS_PER_PLAYER*2)), 0).makeGraphic(1, 1, FlxColor.BLACK);
		gfLockOverlay.alpha = 0.6;
		gfLockOverlay.visible = !PlayState.SONG.gfStrums;
		gfLockOverlay.scrollFactor.x = 0;
		gfLockOverlay.scale.x = GRID_SIZE*GRID_COLUMNS_PER_PLAYER;
		gfLockOverlay.updateHitbox();
		add(gfLockOverlay);
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		if(curSec > 0)
		{
			...
			eventLockOverlay.y = gfLockOverlay.y = prevGridBg.y;
		}
		...
		if(!prevGridBg.visible) eventLockOverlay.y = gfLockOverlay.y = gridBg.y;
		eventLockOverlay.scale.y = gfLockOverlay.scale.y = hei;
		eventLockOverlay.updateHitbox();
		gfLockOverlay.updateHitbox();
...
function updateModEvV1():Void {
	...
	function addSongTab()
	{
		...
		gfGridChkBox = new PsychUICheckBox(objX + 160, objY+40, 'GF has strums', 100, function(){
			PlayState.SONG.gfStrums = gfGridChkBox.checked;
			gfLockOverlay.visible = !PlayState.SONG.gfStrums;
			updateHeads();
		});
		gfGridChkBox.checked = PlayState.SONG.gfStrums;
							...
							try
							{
								...
								if(diffs != null && diffs.length > 0)
								{
									...
									for (diff in diffs)
									{
										...
										{
											var diffChart:SwagSong = ChartingState.parseJSON(File.getContent(chartToFind), songName + diffPostfix);
											if(diffChart != null)
											{
									...
									if(FileSystem.exists(chartToFind))
									{
										var eventsChart:SwagSong = ChartingState.parseJSON(File.getContent(chartToFind), 'events');
										if(eventsChart != null)
										{
...
*/		
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Update (Legacy)...', function()
		{
			...
			fileDialog.open(function()
			{
				...
				try
				{
					...
					var loadedChart:SwagSong = ChartingState.parseJSON(fileDialog.data, filePath, '');
					if(loadedChart == null || !Reflect.hasField(loadedChart, 'song')) //Check if chart is ACTUALLY a chart and valid
					{
						showOutput('Error: File loaded is not a Psych Engine 0.x.x/FNF 0.2.x.x chart.', true);
						return;
					...
					if(!fmt.startsWith('psych_v1'))
					{
						...
						ChartingState.convert(loadedChart);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Exit', function()
		{
			...
			// MasterEditorMenu not present - skipping state switch.
